...
func (s *Services) CommitLease(ctx context.Context, token, oldRootHash, newRootHash string, tag gw.RepositoryTag) (uint64, error) {
	...
	//
	// The lease expiration is passed to the receiver, which re-checks it just
	...
	if err := s.DB.WithLock(ctx, lease.Repository, func() error {
		var err error
		leasePath := lease.CombinedLeasePath()
		finalRev, err = s.Pool.CommitLease(ctx, leasePath, oldRootHash, newRootHash, tag, lease.Expiration)
		return err
	}); err != nil {
		outcome = err.Error()
		return 0, err
