...
##Execute work every X seconds(in separate thread) Idea
    : Have some work executed every X seconds Solution
    : Use a thread with an infinite
      - loop that uses `poll` to
      - Periodically execute the work
      - Stop the thread if requested from the outside

      Details
    : - `Create()`
      - Function allows registering subclasses defined by enum `TypeSelector`
