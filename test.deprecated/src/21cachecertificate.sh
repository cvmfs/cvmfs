...
cvmfs_run_test() {
  ...
  sleep 1
  sudo rm -f /var/lib/cvmfs/lhcb.cern.ch/cvmfschecksum.lhcb.cern.ch
  ls /cvmfs/lhcb.cern.ch >> $logfile || return 4

  sudo cvmfs_talk -i lhcb.cern.ch sqlite memory | grep "certificate disk cache hits/misses 1/0" >> $logfile
  if [ $? -ne 0 ]; then
    return 5
  fi
