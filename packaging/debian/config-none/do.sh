...
usage() {
  echo "Sample script that builds the cvmfs-config-none debian package from source"
...
upstream_version="$(cat ${srctree}/packaging/rpm/cvmfs-config-none.spec | grep '^Version:' | awk '{print $2}')-0"
echo "done"
