...
usage() {
  echo "Sample script that builds the cvmfs-config-egi debian package from source"
...
cp -R ${workdir}/tmp/packaging/debian/config-egi/* ${workdir}/src/cvmfs/debian
cp ${workdir}/tmp/packaging/debian/config-egi/Makefile ${workdir}/src/cvmfs
echo "done"

echo -n "figuring out version number from rpm packaging... "
upstream_version="$(cat ${srctree}/packaging/rpm/cvmfs-config-egi.spec | grep '^Version:' | awk '{print $2}')-0"
echo "done"
