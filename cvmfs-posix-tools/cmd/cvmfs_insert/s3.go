...
type S3Interface interface {
	uploadGivenInfo(reader io.ReadSeeker, partSize int64, retries int, s3Dest string, fileToUpload *pkg.UploadFile, overwriteExisting bool) error
	uploadReaderList(fileName string, readers []pkg.NamedReader, partSize int64, maxConcurrentUploaders int, overwriteExisting bool, fileToUpload *pkg.UploadFile, compressor *pkg.Compressor) error
...
func (s3Interface *S3Manager) uploadGivenInfo(reader io.ReadSeeker, chunkSize int64, retries int, s3Dest string, fileToUpload *pkg.UploadFile, overwriteExisting bool) error {
...
func (s3Interface *S3Manager) uploadReaderListWorker(readers <-chan pkg.NamedReader, errs chan<- error, partSize int64, overwriteExisting bool, fileToUpload *pkg.UploadFile, compressor *pkg.Compressor) {
	for reader := range readers {
		...
		go func() {
			...
			errChan <- s3Interface.uploadGivenInfo(activeReader, partSize, UploadRetries, s3Dest, fileToUpload, overwriteExisting)
		}()
...
func (s3Interface *S3Manager) uploadReaderList(fileName string, readers []pkg.NamedReader, partSize int64, maxConcurrentUploaders int, overwriteExisting bool, fileToUpload *pkg.UploadFile, compressor *pkg.Compressor) error {
	...
	for i := 0; i < t; i++ {
		...
		go func() {
			defer wg.Done()
			s3Interface.uploadReaderListWorker(readersChan, errs, partSize, overwriteExisting, fileToUpload, compressor)
		}()
	}
