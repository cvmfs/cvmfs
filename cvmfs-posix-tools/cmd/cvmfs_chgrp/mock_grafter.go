...
func Mock_check_expected_graft_getter(ctx Context, expecPOG []ExpectedPathGroup) func(db pkg.DB, repo string, priority string, debug bool) (pkg.GraftMetrics, error) {
...
func Mock_check_expected_graft(ctx Context, db pkg.DB, repo string, debug bool, baseDir string, expecPOG []ExpectedPathGroup) (pkg.GraftMetrics, error) {
	...
	for _, pog := range expecPOG {
		...
		case pog.dir:
			...
			if dActual.GetGroup() != pog.group {
				return pkg.GraftMetrics{}, fmt.Errorf("Path: " + dActual.GetName() + " group is not correct. Expected: " + fmt.Sprint(pog.group) + " Actual: " + fmt.Sprint(dActual.GetGroup()))
			}
		case pog.file:
			...
			if fActual.GetGroup() != pog.group {
				return pkg.GraftMetrics{}, fmt.Errorf("Path: " + fActual.GetName() + " group is not correct. Expected: " + fmt.Sprint(pog.group) + " Actual: " + fmt.Sprint(fActual.GetGroup()))
			}
		case pog.sym:
			...
			if lActual.GetGroup() != pog.group {
				return pkg.GraftMetrics{}, fmt.Errorf("Path: " + lActual.GetName() + " group is not correct. Expected: " + fmt.Sprint(pog.group) + " Actual: " + fmt.Sprint(lActual.GetGroup()))
			}
		default:
