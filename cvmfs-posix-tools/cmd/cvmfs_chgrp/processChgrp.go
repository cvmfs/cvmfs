...
func getGroup(ctx Context, groupString string) (int, error) {
	var err error
	var groupId int
	if ctx.referenceSet {
		...
		_, groupId, _, err = pkg.GetPathPerms(fInfo)
		if err != nil {
			return 0, err
		}
	} else {
		if groupId, err = strconv.Atoi(groupString); err != nil {
			grpPtr, err := user.LookupGroup(groupString)
			if err != nil {
				log.Error().Err(err).Str("Group", groupString).Msg("Error looking up group")
				return 0, err
			}
...
func changeFileMode(ctx Context, owner, group, mode int, path, repo *pathlib.Path, pathInfo fs.FileInfo, db pkg.DB) error {
	...
	log.Debug().Str("Path", repo.JoinPath(path).Clean().String()).Int("Group", group).Msg("path now has group")
...
func changeSymMode(ctx Context, owner, group, mode int, path, repo *pathlib.Path, pathInfo fs.FileInfo, db pkg.DB) error {
	fullPath := repo.JoinPath(path)
	...
	log.Debug().Str("Path", repo.JoinPath(path).Clean().String()).Int("Group", group).Msg("sym now has group")
	...
	if ctx.cfg.Repo.DotScheme {
		...
		if isDotLink {
			...
			fileOwner, _, fileMode, err := pkg.GetPathPerms(filePathInfo)
			if err != nil {
				return err
			}
			...
			if err := changeFileMode(ctx, fileOwner, group, fileMode, path.Parent().Join(linkTarget), repo, filePathInfo, db); err != nil {
				return err
			}
		}
	}
...
func changeDirMode(ctx Context, owner, group, mode int, path, repo *pathlib.Path, pathInfo fs.FileInfo, db pkg.DB) error {
	...
	log.Debug().Str("Path", fullPathString).Int("Group", group).Msg("path now has group")
...
func changeObjMode(ctx Context, newGroup int, path, repo *pathlib.Path, db pkg.DB) error {
	fullPath := repo.JoinPath(path)
	fullPathString := fullPath.Clean().String()
	...
	owner, _, mode, err := pkg.GetPathPerms(pathInfo)
	if err != nil {
		return err
	}
	ctx.cfg = pkg.GetBasePathPrefix(ctx.cfg, path)
	...
	case pathlib.IsDir(pathInfo.Mode()):
		...
		if err := changeDirMode(ctx, owner, newGroup, mode, path, repo, pathInfo, db); err != nil {
			return err
		}
		if ctx.recursive {
			...
			for _, child := range childPaths {
				if child.Name() != pkg.CVMFSProtectedFile && child.Name() != pkg.CVMFSAutoProtectedFile {
					if err := changeObjMode(ctx, newGroup, path.Join(child.Name()), repo, db); err != nil {
						return err
					}
				}
			}
	...
	case pathlib.IsFile(pathInfo.Mode()):
		...
		if !ctx.cfg.Repo.DotScheme {
			if err := changeFileMode(ctx, owner, newGroup, mode, path, repo, pathInfo, db); err != nil {
				return err
			}
		}
	case pathlib.IsSymlink(pathInfo.Mode()):
		log.Debug().Str("Path", fullPathString).Msg("Path is sym")
		if err := changeSymMode(ctx, owner, newGroup, mode, path, repo, pathInfo, db); err != nil {
			return err
		}
	default:
...
func processChgrp(ctx Context, group int, paths []*pathlib.Path, repo *pathlib.Path, db pkg.DB) error {
	for _, path := range paths {
		if err := changeObjMode(ctx, group, path, repo, db); err != nil {
			return err
		}
	}
	return nil
