# cvmfs_chgrp
The cvmfs_chgrp tool.

This is a tool designed to be very much like chgrp, but designed to function with the cvmfs file system.

A note about chgrp - it is dot scheme aware. This means that if you are trying to change a dot scheme file, modifying the name of the file will modify the underlying file. The file cannot, however, be directly targeted.
...
This is the cvmfs_chgrp tool, meant to implement chgrp functionality for the CVMFS file  system.
...
Options
  ...
  -f, --file string          chgrp the paths from a line separated list of paths relative to a provided root directory (a la cvmfs_insert). Ex. cvmfs_chgrp <grp> <repo_root> --file <path_file>
  ...
  -R, --recursive            Change files and dirs recursively
      --reference string     Use RFILE's group.
```
