...
func setupTestEnv() func(t *testing.T) {
	...
	linksToCreate := []SymTargetDest{
		...
		SymTargetDest{Dest: "sym_file2ND.txt", Target: "file2.txt"},
	}
	...
	for _, dirPathString := range morePermDirs {
		if err := db.InsertDir(dirPathString, 0775, time.Now().UnixNano(), uid, gid, "user::r-x,group::r-x,other::r-x"); err != nil {
			panic(err)
		}
	}
