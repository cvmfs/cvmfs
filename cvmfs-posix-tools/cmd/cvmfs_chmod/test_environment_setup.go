...
func setupTestEnv(ctx Context) (func(t *testing.T), string, pkg.FileHashData) {
	...
	for _, dirPathString := range aclDirs {
		if err := db.InsertDir(dirPathString, 0555, time.Now().UnixNano(), uid, gid, fmt.Sprintf("user::r-x,group::r-x,user:%s:r-x,mask::r-x,other::r-x", pkg.TestingUnownedUserGroup)); err != nil {
			panic(err)
		}
	}
