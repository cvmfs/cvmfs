...
func chmod(ctx Context, modeStr string, paths []*pathlib.Path, repo *pathlib.Path) (err error) {
	...
	changeSet, err = getModeChangeSet(ctx, modeStr)
	if err != nil {
		return err
	}

	if err = processChomd(ctx, changeSet, paths, repo, db); err != nil {
		return err
	}
...
func launchChmod(ctx Context, modeStr string, filePaths []*pathlib.Path) error {
	var err error
	...
	if err = chmod(ctx, modeStr, relPaths, repo); err != nil {
...
func getAutotunedCtx(ctx Context) Context {
	...
	logLine.Int("num purge workers", newCtx.numHashers).Msg("chmod using these parameters")
	log.Info().Int("Allotted Cores", numAllotedCpus).Msg("Cvmfs chmoding with these parameters")
	return newCtx
}
...
func getInfoFromFlags() (Context, string, []*pathlib.Path, error) {
	...
	flagSet.Usage = func() {
		fmt.Fprintf(os.Stderr, "\nThis is the cvmfs_chmod tool, meant to implement chmod functionality for the CVMFS file "+
			...
			" - it is dot scheme aware. This means that if you are trying to change a dot scheme file, modifying the name of"+
			" the file will modify the underlying file. The file cannot, however, be directly targeted.\n\n"+
			"Memory Estimation:\n"+
	...
	r := flagSet.String("reference", "", "Use RFILE's mode.")
	...
	file := flagSet.StringP("file", "f", "", "chmod the paths from a line separated list of paths relative to a provided root directory (a la cvmfs_insert). Ex. cvmfs_chmod <mode> <repo_root> --file <path_file>")
...
func main() {
	...
	log.Info().Msg("Starting Chmod")
	log.Debug().Msg("Debug Mode")
	// CHANGE OVERRIDE TO FALSE BEFORE DEPLOY
	if err = launchChmod(ctx, modeStr, filePaths); err != nil {
