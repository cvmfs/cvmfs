...
func rmdir(ctx Context, deletePaths []*pathlib.Path, repo *pathlib.Path) (err error) {
...
func launchRmdir(ctx Context, deletePaths []*pathlib.Path) error {
	var err error
	...
	if repo != nil {
		...
		if err = rmdir(ctx, relPaths, repo); err != nil {
			log.Error().Err(err).Msg("Rm processing had a fatal error")
			return err
		}
	} else {
		log.Info().Msg("Nothing to rm, skipping rmdir.")
	}
	return nil
}
...
func getInfoFromFlags() (Context, []*pathlib.Path, error) {
	...
	flagSet.Usage = func() {
		fmt.Fprintf(os.Stderr, "\nThis is the cvmfs_rmdir tool, meant to implement rmdir functionality for the CVMFS file "+
	...
	file := flagSet.StringP("file", "f", "", "Remove the paths from a line separated list of paths relative to a provided root directory (a la cvmfs_insert). Ex. cvmfs_rmdir <repo_root> --file <path_file>")
...
func Execute() error {
	...
	if err = launchRmdir(ctx, dirPaths); err != nil {
