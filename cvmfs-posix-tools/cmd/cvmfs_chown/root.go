...
func chown(ctx Context, ownerString string, paths []*pathlib.Path, repo *pathlib.Path) (err error) {
	...
	owner, group, groupSet, err := getOwnerGroup(ctx, ownerString)
	if err != nil {
		return err
	}

	if err = processChown(ctx, owner, group, groupSet, paths, repo, db); err != nil {
		return err
	}
...
func launchChown(ctx Context, modeStr string, filePaths []*pathlib.Path) error {
	var err error
	...
	if err = chown(ctx, modeStr, relPaths, repo); err != nil {
		log.Error().Err(err).Msg("Chown processing had a fatal error")
		return err
	}
	return nil
}
...
func getAutotunedCtx(ctx Context) Context {
	...
	logLine.Int("num purge workers", newCtx.numHashers).Msg("chown using these parameters")
	log.Info().Int("Allotted Cores", numAllotedCpus).Msg("Cvmfs chowning with these parameters")
	return newCtx
}
...
func getInfoFromFlags() (Context, string, []*pathlib.Path, error) {
	...
	flagSet.Usage = func() {
		fmt.Fprintf(os.Stderr, "\nThis is the cvmfs_chown tool, meant to implement chown functionality for the CVMFS file "+
			" system.\n\nUsage: cvmfs_chown [OPTION]... [OWNER[:GROUP]]... FILE...\ncvmfs_chown [OPTION]... --reference=RFILE FILE...\n"+
			"Tool must be called in a CVMFS directory.\nA note about chown - it is dot scheme aware. This means that if you are trying "+
			"to change a dot scheme file, modifying the name of the file will modify the underlying file. The file cannot, however, be "+
			"directly targeted.\n\n"+
			"Memory Estimation:\n"+
	...
	r := flagSet.String("reference", "", "Use RFILE's owner.")
	...
	file := flagSet.StringP("file", "f", "", "chown the paths from a line separated list of directories relative to a provided root directory (a la cvmfs_insert). Ex. cvmfs_chown <owner:grp> <repo_root> --file <dir_file>")
...
func main() {
	...
	pkg.SetupLogger()
	ctx, modeStr, filePaths, err := getInfoFromFlags()
	if err != nil {
	...
	log.Info().Msg("Starting Chown")
	log.Debug().Msg("Debug Mode")
	// CHANGE OVERRIDE TO FALSE BEFORE DEPLOY
	if err = launchChown(ctx, modeStr, filePaths); err != nil {
