...
void WritableCatalogManager::RemoveNestedCatalog(const string &mountpoint) {
  ...
  // Remove the .cvmfscatalog and .cvmfsautocatalog files
  string root_prefix = nested_catalog->root_prefix().ToString();
  nested_catalog->RemoveEntry(root_prefix + "/.cvmfscatalog");
  nested_catalog->RemoveEntry(root_prefix + "/.cvmfsautocatalog");
...
void WritableCatalogManager::Balance() {
  ...
  for (unsigned i = 0; i < catalog_list.size(); ++i) {
    FixWeight(catalog_list[i]);
  }
}

void WritableCatalogManager::FixWeight(Catalog* catalog) {
  ...
  if (catalog->GetNumEntries() < min_weight_ &&
    ...
    LogCvmfs(kLogCatalog, kLogStdout,
             "deleting an autogenerated catalog in '%s'", catalog_path.c_str());
