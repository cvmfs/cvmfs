...
void GatewayS3Uploader::FinalizeStreamedUpload(UploadStreamHandle *handle,
  ...
  GatewayStreamHandle *gw_handle = dynamic_cast<GatewayStreamHandle *>(handle);
  assert(gw_handle);
  ...
  LogCvmfs(kLogUploadS3, kLogDebug, "GatewayS3: routing data %s to S3",
           content_hash.ToString().c_str());
  ...
  const std::string object_key = repo_alias_ + "/data/"
  ...
  s3fanout::JobInfo *info = new s3fanout::JobInfo(
      object_key,
      const_cast<void *>(static_cast<void const *>(gw_handle->commit_callback)),
      buf);
...
void *GatewayS3Uploader::MainCollectResults(void *data) {
  LogCvmfs(kLogUploadS3, kLogDebug, "GatewayS3 S3 WorkerThread started.");
  GatewayS3Uploader *uploader = reinterpret_cast<GatewayS3Uploader *>(data);
  ...
  LogCvmfs(kLogUploadS3, kLogDebug, "GatewayS3 S3 WorkerThread finished.");
  return NULL;
}
