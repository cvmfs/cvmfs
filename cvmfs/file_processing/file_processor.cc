...
void FileProcessor::FileDone(File *file) {
  ...
  for (; i != iend; ++i) {
    ...
    shash::Any chunk_content_hash = current_chunk->content_hash();
    chunk_content_hash.suffix = shash::kSuffixPartial;
    resulting_chunks.PushBack(FileChunk(chunk_content_hash,
