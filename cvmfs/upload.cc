...
Spooler *Spooler::Construct(const SpoolerDefinition &spooler_definition,
  ...
  result->uploader_.reset(uploader);
  if (statistics != NULL) {
    result->uploader_->InitCounters(statistics);
  }
  // configure the file processor context
  result->ingestion_pipeline_.reset(
      new IngestionPipeline(result->uploader_.get(), spooler_definition));
  result->ingestion_pipeline_->RegisterListener(&Spooler::ProcessingCallback,
                                                result);
