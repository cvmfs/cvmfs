...
bool ServerTool::InitDownloadManager(const bool follow_redirects,
  ...
  download_manager_ = std::unique_ptr<download::DownloadManager>(
...
bool ServerTool::InitSignatureManager(const std::string &pubkey_path,
  ...
  signature_manager_ = std::unique_ptr<signature::SignatureManager>(
...
manifest::Manifest *ServerTool::FetchRemoteManifest(
  ...
  if (retval != manifest::kFailOk) {
    ...
    manifest = std::unique_ptr<manifest::Manifest>(
        new manifest::Manifest(*manifest_ensemble.manifest));
  }
