...
struct ChunkTables {
  ...
  inline void Lock() {
    int const retval = pthread_mutex_lock(lock);
    assert(retval == 0);
  }

  inline void Unlock() {
    int const retval = pthread_mutex_unlock(lock);
    assert(retval == 0);
  }
...
class SimpleChunkTables : SingleCopy {
 ...
 private:
  inline void Lock() {
    int const retval = pthread_mutex_lock(lock_);
    assert(retval == 0);
  }

  inline void Unlock() {
    int const retval = pthread_mutex_unlock(lock_);
    assert(retval == 0);
  }
