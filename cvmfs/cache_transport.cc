...
const char
    *CacheTransport::kEnvReadyNotifyFd = "__CVMFS_CACHE_EXTERNAL_PIPE_READY__";
...
CacheTransport::Frame::~Frame() { Reset(0); }
...
bool CacheTransport::RecvFrame(CacheTransport::Frame *frame) {
  ...
  if (has_attachment) {
    ...
    void *ptr_attachment = reinterpret_cast<char *>(buffer) + kInnerHeaderSize
                           + msg_size;
