...
CommandCheck::CommandCheck()
  ...
  bool CommandCheck::CompareEntries(const catalog::DirectoryEntry &a,
    ...
    if (diffs & Difference::kChecksum) {
      LogCvmfs(kLogCvmfs, kLogStderr, "content hashes differ: %s / %s",
               a.checksum().ToString().c_str(),
               b.checksum().ToString().c_str());
      retval = false;
    }
    ...
    if (diffs & Difference::kHasXattrsFlag) {
      LogCvmfs(kLogCvmfs, kLogStderr,
               ...
               "(%s / %s)",
               a.HasXattrs(), b.HasXattrs(), a.name().c_str(),
               b.name().c_str());
      retval = false;
    }
    if (!is_transition_point) {
  ...
  bool CommandCheck::Exists(const string &file, zip::Algorithm decomp_alg) {
    if (!is_remote_) {
      ...
      download::JobInfo head(&url, decomp_alg, false, /*hash=*/NULL,
                             /*sink=*/NULL);
  ...
  string CommandCheck::FetchPath(const string &path) {
    ...
    if (is_remote_) {
      ...
      download::JobInfo download_job(
          &url, zip::DecompressionAlg::kNoCompression, false, NULL, &filesink);
  ...
  bool CommandCheck::InspectReflog(const shash::Any &reflog_hash,
    ...
    if (computed_hash != reflog_hash) {
      LogCvmfs(kLogCvmfs, kLogStderr,
               ...
               computed_hash.ToString().c_str(),
               reflog_hash.ToString().c_str());
      unlink(reflog_path.c_str());
      return false;
    }

    const UniquePtr<manifest::Reflog> reflog(
  ...
  bool CommandCheck::Find(const catalog::Catalog *catalog,
    ...
    for (unsigned i = 0; i < entries.size(); ++i) {
      ...
      const bool entry_needs_check = !entries[i].checksum().IsNull()
                                     ...
                                     &&
                                     // fallback cli option can force the entry
                                     // to be checked
                                     (no_duplicates_map_
      ...
      if ((entries[i].linkcount() > 1) && !entries[i].IsDirectory()) {
        if (entries[i].hardlink_group() == 0) {
          ...
          if (hardlink_group == hardlinks.end()) {
            ...
            if (!CompareEntries(entries[i], (hardlink_group->second)[0],
                                false)) {
      ...
      if (entries[i].IsDirectory()) {
        ...
        if (entries[i].IsNestedCatalogMountpoint()
          ...
          if (!catalog->FindNested(mountpoint, &tmp, &tmp2)) {
            LogCvmfs(kLogCvmfs, kLogStderr,
                     "nested catalog at %s not registered", full_path.c_str());
            retval = false;
          }
        ...
        if (entries[i].symlink().GetLength() > 0) {
          LogCvmfs(kLogCvmfs, kLogStderr,
                   "special file with non-zero symlink at %s",
                   full_path.c_str());
          retval = false;
        }
      } else {
      ...
      if (entries[i].IsChunkedFile()) {
        ...
        catalog->ListPathChunks(full_path, entries[i].hash_algorithm(),
                                &chunks);
        ...
        if (chunks.size() == 0) {
          LogCvmfs(kLogCvmfs, kLogStderr,
                   "no file chunks found for big file %s", full_path.c_str());
          retval = false;
        }
  ...
  string CommandCheck::DecompressPiece(
      const shash::Any catalog_hash, zip::ExpectedContentFormat expected_fmt) {
  ...
  catalog::Catalog *CommandCheck::FetchCatalog(const string &path,
    ...
    tmp_file = DecompressPiece(catalog_hash,
                               zip::ExpectedContentFormat::kSQLite3);
  ...
  bool CommandCheck::FindSubtreeRootCatalog(
      const string &subtree_path, shash::Any *root_hash, uint64_t *root_size) {
  ...
  bool CommandCheck::InspectTree(
      ...
      catalog::DeltaCounters *computed_counters,
      bool *pruned_subtree) {
    LogCvmfs(kLogCvmfs, kLogStdout | kLogInform,
             "[inspecting catalog] %s at %s", catalog_hash.ToString().c_str(),
             path == "" ? "/" : path.c_str());
    ...
    if (is_nested_catalog) {
      ...
      if (!root_entry.IsNestedCatalogRoot()) {
        LogCvmfs(kLogCvmfs, kLogStderr,
                 "nested catalog root expected but not found (%s)",
                 path.c_str());
        retval = false;
      }
    } else {
      if (root_entry.IsNestedCatalogRoot()) {
        LogCvmfs(kLogCvmfs, kLogStderr,
                 "nested catalog root found but not expected (%s)",
                 path.c_str());
        retval = false;
      }
    }
    ...
    for (catalog::Catalog::NestedCatalogList::const_iterator
      ...
      // on the Stratum-1 (catalog + objects absent), so we must not descend
      // into it.  Skipping recursion here also means the aggregate counters for
      // this catalog will be lower than the stored ones; record that so the
      // counter comparison can be relaxed below.
    ...
    // what we could recompute, so the comparison would always fail.  Skip it
    // for affected catalogs; self counters and structure are still fully
    // verified.
    if (local_pruned) {
      LogCvmfs(kLogCvmfs, kLogStdout,
  ...
  int CommandCheck::Main(const swissknife::ArgumentList &args) {
    ...
    if (!manifest->meta_info().IsNull()) {
      ...
      tmp_file = DecompressPiece(manifest->meta_info(),
                                 zip::ExpectedContentFormat::kJSON);
      if (tmp_file == "") {
