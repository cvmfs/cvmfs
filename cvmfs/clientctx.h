...
class ClientCtx {
 public:
  struct ThreadLocalStorage {
    ThreadLocalStorage(uid_t u, gid_t g, pid_t p, InterruptCue *ic, uint64_t http_txn_id, uint32_t http_txn_seq)
      : uid(u), gid(g), pid(p), interrupt_cue(ic), http_txn_id(http_txn_id), http_txn_seq(http_txn_seq),  is_set(true) { }
    uid_t uid;
    gid_t gid;
    pid_t pid;
  ...
  void Set(uid_t uid, gid_t gid, pid_t pid, InterruptCue *ic, uint64_t http_txn_id, uint32_t http_txn_seq);
  void Unset();
  void Get(uid_t *uid, gid_t *gid, pid_t *pid, InterruptCue **ic, uint64_t *http_txn_id, uint32_t *http_txn_seq);
  bool IsSet();
...
class ClientCtxGuard {
 public:
  ClientCtxGuard(uid_t uid, gid_t gid, pid_t pid, InterruptCue *ic, uint64_t http_txn_id, uint32_t http_txn_seq)
    ...
    , old_http_txn_seq_(0)
  {
    ...
    if (old_ctx->IsSet()) {
      ...
      old_ctx->Get(&old_uid_, &old_gid_, &old_pid_, &old_interrupt_cue_, &old_http_txn_id_, &old_http_txn_seq_);
    }
  ...
  ~ClientCtxGuard() {
    ...
    if (set_on_construction_) {
      ctx->Set(old_uid_, old_gid_, old_pid_, old_interrupt_cue_, old_http_txn_id_, old_http_txn_seq_);
    } else {
      ctx->Unset();
    }
  }
