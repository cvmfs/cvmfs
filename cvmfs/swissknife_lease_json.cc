    ...
    if (status == "ok") {
      ...
      if (token != NULL) {
        const JSON *rev = JsonDocument::SearchInObject(
            reply->root(), "revision",
            nlohmann::json::value_t::number_integer);  // TODO FIXME: make the json lib uint64 aware
        if (rev != NULL) {
          *current_revision = (uint64_t)rev->get<int>();
        }
        const JSON *hash = JsonDocument::SearchInObject(
            reply->root(), "root_hash", nlohmann::json::value_t::string);
        if (hash != NULL) {
...
LeaseReply ParseAcquireReply(const CurlBuffer &buffer,
  ...
  if (result != NULL) {
    ...
    if (status == "ok") {
      ...
      const JSON *time_remaining = JsonDocument::SearchInObject(
          reply->root(), "time_remaining", nlohmann::json::value_t::number_integer);
      if (time_remaining != NULL) {
