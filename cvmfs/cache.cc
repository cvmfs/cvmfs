...
int64_t PosixCacheManager::Write(const void *buf, uint64_t size, void *txn) {
  ...
  if ((transaction->expected_size != kSizeUnknown) && (transaction->size + size > transaction->expected_size)) {
    LogCvmfs(kLogCache, kLogDebug, "Transaction size (%d) > expected size (%d)", transaction->size + size, transaction->expected_size);
    return -ENOSPC;
  }
