...
class IntegerMap {
 ...
 public:
  ...
  T Map(const T k) const {
    assert(IsValid());
    typename map_type::const_iterator const i = map_.find(k);
    if (i != map_.end()) {
      return i->second;
    }
 ...
 protected:
  bool ReadFromFile(const std::string &path) {
    ...
    sanitizer::IntegerSanitizer const int_sanitizer;
    ...
    while (GetLineFile(fmap, &line)) {
      ...
      if (components.size() != 2                ||
        ...
        ret = fclose(fmap);
        if (ret != 0) {
          LogCvmfs(kLogUtility, kLogDebug, "failedto close file %s", path.c_str());
        }
