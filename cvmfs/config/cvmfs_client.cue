...
#ClientConfig: {
	...
	CVMFS_AUTO_UPDATE: bool | *false @description("If set to no, disables the automatic update of file catalogs.")
	CVMFS_AUTHZ_HELPER?: #AbsPath @description("Full path to an authz helper, overwrites the helper hint in the catalog.")
	CVMFS_AUTHZ_SEARCH_PATH?: #AbsPath @description("Full path to the directory that contains the authz helpers.")
	...
	CVMFS_BLACKLIST?: #AbsPath @description("File name of the blacklist that denies mounting any revision < revision N. Format: <REPO N where REPO is the repository name, N is the revision number, and the two parts are separated by whitespace. Note: no extra characters are allowed after N, not even whitespace.")
	...
	CVMFS_CACHE_PRIMARY?: =~"^[A-Za-z0-9_]+$" @description("Type of cache to use. By default it is posix. (see also Advanced Cache Configuration)")
	CVMFS_CACHE_REFCOUNT: bool | *false @description("If set to yes, deduplicate open file descriptors by refcounting.")
	...
	CVMFS_KCACHE_TIMEOUT?: int @description("Timeout in seconds for path names and file attributes in the kernel file system buffers.")
	...
	CVMFS_MAX_RETRIES?: #UInt @description("Maximum number of retries for a given proxy/host combination.")
	...
	CVMFS_MEMCACHE_SIZE?: #PosInt @description("Size of the CernVM-FS metadata memory cache in Megabytes.")
	CVMFS_MOUNT_DIR: #AbsPath @description("Directory where CernVM-FS is mounted to. Default is /cvmfs and cannot be overwritten.")
	...
	CVMFS_NFILES: #PosInt @description("Maximum number of open file descriptors that can be used by the CernVM-FS process.")
	...
	CVMFS_QUOTA_LIMIT: #QuotaLimit @description("Soft-limit of the cache in Megabyte.")
	CVMFS_RELOAD_SOCKETS: #AbsPath @description("Directory of the sockets used by the CernVM-FS loader to trigger hotpatching/reloading.")
	...
	CVMFS_SHARED_CACHE: bool @description("If set to no, makes a repository use an exclusive cache.")
	...
	CVMFS_STRICT_MOUNT: bool @description("If set to yes, mount only repositories that are listed in CVMFS_REPOSITORIES.")
	...
	CVMFS_TIMEOUT: #UInt @description("Timeout in seconds for HTTP requests with a proxy server.")
	CVMFS_TIMEOUT_DIRECT: #UInt @description("Timeout in seconds for HTTP requests without a proxy server.")
