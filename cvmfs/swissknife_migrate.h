...
class CommandMigrate : public Command {
 ...
 public:
  ...
  struct PendingCatalog {
    ...
    inline shash::Any GetOldContentHash() const { return old_catalog->hash(); }
  ...
  class PendingCatalogMap
      : public std::map<std::string, const PendingCatalog *>,
        public Lockable { };
  ...
  class AbstractMigrationWorker : public ConcurrentWorker<DerivedT> {
   ...
   protected:
    ...
    catalog::WritableCatalog *GetWritable(
 ...
 public:
  ...
  virtual std::string GetDescription() const {
    return "CernVM-FS catalog repository migration \n"
           "This command migrates the whole catalog structure of a given "
           "repository";
  }
 ...
 protected:
  ...
  bool LoadCatalogs(const shash::Any &manual_root_hash,
    ...
    if (!manifest_upstream_->history().IsNull()) {
      retval = object_fetcher->FetchHistory(&history_upstream_,
                                            manifest_upstream_->history());
      if (retval != ObjectFetcherFailures::kFailOk) {
