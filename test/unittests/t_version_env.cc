...
TEST_F(T_VersionEnv, SetupGlobalEnvironmentParams) {
  ...
  EXPECT_STREQ(cvmfs_version, CVMFS_VERSION) << "CVMFS_VERSION should match compile-time constant";
  
  // Check CVMFS_VERSION_NUMERIC is set and calculated correctly
  const char* cvmfs_version_numeric = getenv("CVMFS_VERSION_NUMERIC");
  ASSERT_NE(cvmfs_version_numeric, nullptr) << "CVMFS_VERSION_NUMERIC should be set";
  
  int numeric_version = atoi(cvmfs_version_numeric);
  int expected_numeric = CVMFS_VERSION_MAJOR * 10000 + CVMFS_VERSION_MINOR * 100 + CVMFS_VERSION_PATCH;
  EXPECT_EQ(numeric_version, expected_numeric) 
    << "CVMFS_VERSION_NUMERIC should be calculated as major*10000 + minor*100 + patch";
  
  // For version 2.13.2, this should be 21302
  EXPECT_EQ(numeric_version, 21302) << "For version 2.13.2, numeric should be 21302";
...
TEST_F(T_VersionEnv, VersionNumericFormat) {
  ...
  int test_cases[][4] = {
    // major, minor, patch, expected_numeric
    {2, 13, 2, 21302},
    {3, 0, 0, 30000},
    {2, 15, 10, 21510},
    {1, 2, 3, 10203}
