...
class T_History : public ::testing::Test {
 ...
 private:
  ...
  void CloseHistory(SqliteHistory *history) { delete history; }
  ...
  bool IsMocked(const type<MockHistory> type_specifier) const { return true; }
 ...
 protected:
  ...
  bool NeedsSandbox() const { return NeedsSandbox(type<HistoryT>()); }

  bool IsMocked() const { return IsMocked(type<HistoryT>()); }
  ...
  History::Tag GetDummyTag(const std::string &name = "foobar",
  ...
  bool TagsEqual(const History::Tag &lhs, const History::Tag &rhs) const {
    return (lhs.name == rhs.name) && (lhs.root_hash == rhs.root_hash)
           && (lhs.size == rhs.size) && (lhs.revision == rhs.revision)
...
template<class HistoryT>
const std::string
    T_History<HistoryT>::history_v1_r0_path = string(
                                                  T_History<HistoryT>::sandbox)
                                              + "/history_v1_r0";

template<class HistoryT>
const std::string
    T_History<HistoryT>::history_v1_r1_path = string(
                                                  T_History<HistoryT>::sandbox)
                                              + "/history_v1_r1";

template<class HistoryT>
const std::string
    T_History<HistoryT>::history_v1_r2_path = string(
                                                  T_History<HistoryT>::sandbox)
                                              + "/history_v1_r2";

template<class HistoryT>
const std::string T_History<HistoryT>::history_v1_r0 =
    "eJztlb1v00AUwM8+N00QnVBkul0main9cBxBUJeayqoiSgHHAx1QdLEv8Sn+qn2uqJjKH8HM/"
    "4SQ"
    "mJnpwgALA3dOkVMkBAsClfvp3tO7p3vn92T7vdHTQ8oImqZ5jBmygAYUBewhBABocFkBNSoXbW"
    "mv"
    "gF/TAJsfjDX4FWhqCmAHvlVT9eNvxEmuJy/"
    "hqt7pKOc2w5OIZHmakZxRUtSWtu86tucgz35w6KDa"
    "jzbm5Ax5zjOvi05xVJJLe//"
    "x0chz7eGRh7L5eOn8E3f4yHaP0UPnuIo1DEtr6AcdBdAkIC+Kk4h/"
    "+GNcsrTaL4WOzdpeOd9WVvV2W3m1SJnhWSFEvZKm8KCNBMffswpxEV6aOTmlBU0TxFN0Dhy3ix"
    "Bi"
    "NCb8mjirnX6Ik4REtSMghZ/TjInQxU0/"
    "1Fo99EqVIgHDuKM29N32z8oUUWNTaMjfh7bGlQJfA74k"
    "Esk/"
    "Qch7zvOW1oxZXibzwLT8+4NJf9If+CTAfoCt3oAQ3MfWzqB31+KYPd+8N3ozfBdhxjsLyspJ"
    "RIuQBKhIcFaEKesi0QP4kKc+jqIzVGYBPxrcAOL//wL4kkgk/wdNqLeq3iLmvwrfA/"
    "iJK4lEci24"
    "rcL29vQkT/w0mdLZJpnRrUrKmwpcv1X4IYmxubUj5r8KLwC8gJ//"
    "ds4SieQP0oRtRfQEtQXXW4sW"
    "8A15t8pp";

template<class HistoryT>
const std::string T_History<HistoryT>::history_v1_r1 =
    "eJztl09r1EAUwCd5absVS6FlWXoQpniwoX/"
    "Y7KbZLaK4lqUUa9VtDvYg28lkYkKzSZqZLa2e6kfw"
    "W3n21EMvxU/gQS8ezOy2ZFcoehDEkh/"
    "M5M1j3j+SN0P2Xu0EgmEvTntE4DrSkKKgJxgjhGayMYFy"
    "1GxoI2sF/"
    "Z4ZtHqpz8APpKkHCObgs3qgXvyBXcEt4T1MVRYXlbOWIE7IkjROWCoCxnNJ2+y0W3Yb"
    "262nO22c6/HSITvFdvu1vYKPSdhnV/Lmi909u9Pa3rVxctgd2f+ys/"
    "281dnHz9r7A1tdr2uTla1F"
    "BQWRy074UZh96V3SF/FgPWLaNXJ54uyxMlUpl5UP3UHKgrzlcqhjaUoNXopI7zorn3D/"
    "SkzZccCD"
    "OMJZiu2tdmcFYyyCHsvc9JJcSX0SRSzMFS7jNA0SIU2Hnnjwjo16GS99kMNY0TIfXX+"
    "gTlYelm+q"
    "Wlp1DTlD9no02eQq3EPwMZsKCgpyuFrS3yhaKVBF2o8OVxPZ2nGfNxuWZXoNt05N0lgnTbrhWT"
    "VW"
    "J2bD2bBqtFo1XMtZrxHV5rN60nfCgPvMxRE7EVjEeOBsBcuezC7dgJIwPMX9xCWCuU0UKqWKDN"
    "ob"
    "BvVMx3Isz2IbTeZQ5hGTsapBTcOxTIcQ5jhmbZ0yF2z+KAgzF1zgPCSPSML9WNwY7Q6S/"
    "f8Fwfds"
    "KigouHXMga6MH2BqCSrTA5W8/"
    "wE+IfgKF9mjoKDgf2QOoHzfO0ojj9I1ytJojfrzKizPcuqzHule"
    "/5UYdxVYmB8qjbWqvP8BzhGcwze4/NdFFBQU/BVKUFbkcQDzsKz8cgao07AwPdT9BBspCA8=";

template<class HistoryT>
const std::string T_History<HistoryT>::history_v1_r2 =
    "eJztl79P20AUx8++kAQEaqU0shiQDiEEFpDiOI5jVaqaUpci6K+"
    "QIahF1mFfiIXjBNtBBYaWTl36"
    "B6BuFWP7T7RD924dOvQP6IJUtWPvElCSSgikTqD7yHc+v/"
    "g9f5917y5efbriRgRVG0EdR0gFMSAI"
    "4A5CAIy9AwAkQJfYSTtFAOdCY2RGro/"
    "APyAuHAF4H34Wn8Mx4UicPN+Xc8rLgYQ0MSEcmBHe8EhA"
    "7F3bI9aG6/"
    "cM4wsls1g2Ubl4d8VEPT+g6RoOa6hsVsqzqOrhzRAtPSqbi2ZpFiG08PjRarlUpBbU"
    "3LLadz4pLT0sltbQsrnW8ZVlLR6XliYE4PoOeRFue3TGWLgVNdrXVs/"
    "DLKXnInFwGyakdFp4bbWF"
    "R/"
    "TZrMX6pDILmvZxnZxo7JEbkB03dBt+r+LIrRMapt7sGu0a9n3idQ0OCe3AbUbMtRMpdPfI2Xm3"
    "NfTlzfTI8lQsLt1Kn5U387IU1g/"
    "sCwlpfFw4KLYTbQaNJgkil4TdkdiXdNeOprfI7onIHey1Tt9C"
    "v8Ce+/"
    "tkUl9ZVsW4tDh+lsquq6V0x5BOqySbWyO0QfgFwGP4nZ44HM7l4waE6anqduBjz7VJxiaB"
    "n7FrKRHOXAvtGqlj63QtzQ4LcDTVMSqZ+SHA6v8bgL/"
    "hT3ricDhXgRScEf4pfXEQjg52bEmYFthy"
    "Adn+L8IygO9px+Fw/"
    "pvlw7mktC7GkpOxKGj5W2o2W9AcXDA0HeeMqobzzvwGxrqKFZzLzhNNy+m6"
    "YjvJ/"
    "crxzbd2KwiIH6EHZvEerLwCzw5nkzKLpnaizTVZPTdaYY7YBa2KFVXXHUM3NIPGdHS9kC/Y"
    "+ZyS13RbLRiqoST3KsdTnxxSxS0vQi3fadCPrmCTRCw62/9F+APAX7TjcDhXixSUxf51Y/"
    "jDEJQ6"
...
TYPED_TEST(T_History, RollbackToOldTag) {
  ...
  if (gone[0].name == "also_rofl") {  // order of rev 8 tags is undefined
    EXPECT_EQ("also_rofl", gone[0].name);
    EXPECT_EQ(8u, gone[0].revision);
    EXPECT_EQ("rofl", gone[1].name);
    EXPECT_EQ(8u, gone[1].revision);
  } else {
    EXPECT_EQ("rofl", gone[0].name);
    EXPECT_EQ(8u, gone[0].revision);
    EXPECT_EQ("also_rofl", gone[1].name);
    EXPECT_EQ(8u, gone[1].revision);
  }
  EXPECT_EQ("lol", gone[2].name);
  EXPECT_EQ(5u, gone[2].revision);
  EXPECT_EQ("moep", gone[3].name);
  EXPECT_EQ(4u, gone[3].revision);
...
TYPED_TEST(T_History, ListTagsAffectedByRollback) {
  ...
  if (gone[3].name == "moep_duplicate") {  // undefined order of same revision
    EXPECT_EQ("moep_duplicate", gone[3].name);
    EXPECT_EQ(4u, gone[3].revision);
    EXPECT_EQ("moep", gone[4].name);
    EXPECT_EQ(4u, gone[4].revision);
  } else {
    EXPECT_EQ("moep", gone[3].name);
    EXPECT_EQ(4u, gone[3].revision);
    EXPECT_EQ("moep_duplicate", gone[4].name);
    EXPECT_EQ(4u, gone[4].revision);
  }
  EXPECT_EQ("test_release", gone[5].name);
  EXPECT_EQ(3u, gone[5].revision);
  EXPECT_EQ("bar", gone[6].name);
  EXPECT_EQ(2u, gone[6].revision);
...
TYPED_TEST(T_History, EmptyRecycleBin) {
  ...
  dummy_foo.name = "foo";
  dummy_foo.root_hash = shash::MkFromHexPtr(
...
TYPED_TEST(T_History, RollbackAndRecycleBin) {
  ...
  dummy_bar.name = "bar";
  dummy_bar.root_hash = shash::MkFromHexPtr(
  ...
  dummy_baz.name = "baz";
  dummy_baz.root_hash = shash::MkFromHexPtr(
...
TYPED_TEST(T_History, InsertBranchedTags) {
  ...
  History::Tag tag_foo;
  tag_foo.name = "foo";
  tag_foo.root_hash = shash::MkFromHexPtr(
      shash::HexPtr("5207a527a4fee2d655c67415aa1979f1d2753f96"),
      shash::kSuffixCatalog);
  tag_foo.revision = 1;
  ...
  History::Tag tag_bar;
  tag_bar.name = "bar";
  tag_bar.root_hash = shash::MkFromHexPtr(
      shash::HexPtr("19552496e1e5c63aefaf5d4e05a8c248a1d82663"),
      shash::kSuffixCatalog);
  tag_bar.revision = 2;
  tag_bar.branch = "br1";
  ...
  tag_invalid.root_hash = shash::MkFromHexPtr(
      shash::HexPtr("19552496e1e5c63aefaf5d4e05a8c248a1d82663"),
      shash::kSuffixCatalog);
...
TYPED_TEST(T_History, PruneBranches) {
  ...
  History::Tag tag_baz;
  tag_baz.name = "baz";
  tag_baz.root_hash = shash::MkFromHexPtr(
  ...
  tag_baz_deep.root_hash = shash::MkFromHexPtr(
