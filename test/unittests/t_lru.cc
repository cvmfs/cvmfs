...
TEST(T_LruCache, PopOldest) {
  ...
  bool found;
  int k;
  found = cache.Lookup(5, &v); EXPECT_TRUE(found);
  found = cache.Lookup(4, &v); EXPECT_TRUE(found);
  found = cache.Lookup(7, &v); EXPECT_TRUE(found);
  found = cache.Lookup(3, &v); EXPECT_TRUE(found);
  found = cache.Lookup(5, &v); EXPECT_TRUE(found);
  found = cache.Lookup(9, &v); EXPECT_TRUE(found);
  found = cache.Lookup(6, &v); EXPECT_TRUE(found);
  found = cache.Lookup(8, &v); EXPECT_TRUE(found);
  found = cache.Lookup(1, &v); EXPECT_TRUE(found);
  found = cache.Lookup(4, &v); EXPECT_TRUE(found);
  found = cache.Lookup(2, &v); EXPECT_TRUE(found);
  found = cache.Lookup(5, &v); EXPECT_TRUE(found);
  found = cache.Lookup(9, &v); EXPECT_TRUE(found);
  found = cache.Lookup(6, &v); EXPECT_TRUE(found);
  found = cache.Lookup(4, &v); EXPECT_TRUE(found);
  ...
  found = cache.PopOldest(&k, &v);  EXPECT_TRUE(found); EXPECT_EQ(7, k);
  found = cache.PopOldest(&k, &v);  EXPECT_TRUE(found); EXPECT_EQ(3, k);
  found = cache.PopOldest(&k, &v);  EXPECT_TRUE(found); EXPECT_EQ(8, k);
  found = cache.PopOldest(&k, &v);  EXPECT_TRUE(found); EXPECT_EQ(1, k);
  found = cache.PopOldest(&k, &v);  EXPECT_TRUE(found); EXPECT_EQ(2, k);

  EXPECT_FALSE(cache.IsEmpty());
  EXPECT_FALSE(cache.IsFull());

  found = cache.Lookup(6, &v); EXPECT_TRUE(found);
  found = cache.Lookup(8, &v); EXPECT_FALSE(found);
  found = cache.Lookup(1, &v); EXPECT_FALSE(found);
  found = cache.Lookup(5, &v); EXPECT_TRUE(found);
  found = cache.Lookup(9, &v); EXPECT_TRUE(found);
  found = cache.Lookup(7, &v); EXPECT_FALSE(found);
  found = cache.Lookup(3, &v); EXPECT_FALSE(found);
  found = cache.Lookup(4, &v); EXPECT_TRUE(found);
  found = cache.Lookup(2, &v); EXPECT_FALSE(found);

  found = cache.PopOldest(&k, &v);  EXPECT_TRUE(found); EXPECT_EQ(6, k);
  found = cache.PopOldest(&k, &v);  EXPECT_TRUE(found); EXPECT_EQ(5, k);

  found = cache.Lookup(1, &v); EXPECT_FALSE(found);
  found = cache.Lookup(2, &v); EXPECT_FALSE(found);
  found = cache.Lookup(3, &v); EXPECT_FALSE(found);
  found = cache.Lookup(4, &v); EXPECT_TRUE(found);
  found = cache.Lookup(5, &v); EXPECT_FALSE(found);
  found = cache.Lookup(6, &v); EXPECT_FALSE(found);
  found = cache.Lookup(7, &v); EXPECT_FALSE(found);
  found = cache.Lookup(8, &v); EXPECT_FALSE(found);
  found = cache.Lookup(9, &v); EXPECT_TRUE(found);

  found = cache.PopOldest(&k, &v);  EXPECT_TRUE(found); EXPECT_EQ(4, k);
  found = cache.PopOldest(&k, &v);  EXPECT_TRUE(found); EXPECT_EQ(9, k);
  found = cache.PopOldest(&k, &v);  EXPECT_FALSE(found);
