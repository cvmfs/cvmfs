...
class T_ObjectFetcher : public ::testing::Test {
 ...
 protected:
  ...
  void SignString(std::string *str,
    ...
    shash::HashMem(reinterpret_cast<const unsigned char *>(str->data()),
                   str->length(), &hash);
  ...
  void WriteManifest() {
    ...
    std::unique_ptr<manifest::Manifest> manifest(
        new manifest::Manifest(root_hash, catalog_size, root_path));
  ...
  void WriteWhitelist() {
    ...
    whitelist += "E"
                 + MakeWhitelistTimestamp(time(0) + 30 /*days*/ * 24 * 60 * 60)
                 + "\n";
    whitelist += "N" + fqrn + "\n";
  ...
  void FinalizeExternalManagers() { signature_manager_.Fini(); }
  ...
  void CreateHistory(shash::Any *content_hash,
    ...
    return CreateHistory(type<ObjectFetcherT>(), content_hash,
                         previous_revision);
  }

  void CreateCatalog(shash::Any *content_hash, const std::string &root_path) {
    return CreateCatalog(type<ObjectFetcherT>(), content_hash, root_path);
  }
 ...
 private:
  ...
  void CreateReflog(const type<LocalObjectFetcher<> > type_spec,
    ...
    std::unique_ptr<manifest::Reflog> reflog(
        manifest::Reflog::Create(reflog_path, fqrn));
    content_hash->algorithm = shash::kSha1;
  ...
  void CreateSandboxHistory(shash::Any *content_hash,
    ...
    history::SqliteHistory *history = ObjectFetcherT::HistoryTN::Create(
        tmp_path, fqrn);
  ...
  void CreateSandboxCatalog(shash::Any *content_hash,
    ...
    catalog::CatalogDatabase *catalog_db = catalog::CatalogDatabase::Create(
        tmp_path);
    ...
    const bool success = catalog_db->InsertInitialValues(
                             root_path, volatile_content, "", root_entry)
                         && catalog_db->SetProperty("revision",
                                                    catalog_revision);
  ...
  void InsertIntoStorage(const std::string &txn_path,
    ...
    ASSERT_EQ(0, rename(txn_path.c_str(), res_path.c_str()))
        << "failed to rename() compressed file " << txn_path << " to "
        << res_path << " with content hash " << content_hash.ToString()
        << " (errno: " << errno << ")";
  }
...
template<class ObjectFetcherT>
const shash::Any T_ObjectFetcher<ObjectFetcherT>::broken_history_hash = h(
    "b904b56ffd47ba89f26d5a887063b4a1fbfb9307", shash::kSuffixHistory);

template<class ObjectFetcherT>
const shash::Any T_ObjectFetcher<ObjectFetcherT>::broken_catalog_hash = h(
    "c4818243550ae6a46f55602043eb23f8c4f97910", shash::kSuffixCatalog);
...
typedef ::testing::Types<MockObjectFetcher, LocalObjectFetcher<>,
                         HttpObjectFetcher<> >
    ObjectFetcherTypes;
...
TYPED_TEST(T_ObjectFetcher, InitializeSlow) {
  ...
  EXPECT_TRUE(object_fetcher.get() != nullptr);
...
TYPED_TEST(T_ObjectFetcher, FetchManifestSlow) {
  ...
  typename TypeParam::Failures retval = object_fetcher->FetchManifest(
      &manifest);
  ...
  std::unique_ptr<manifest::Manifest> manifest_ptr;
  EXPECT_FALSE(manifest_ptr.get() != nullptr);
  typename TypeParam::Failures retval2 = object_fetcher->FetchManifest(
      &manifest_ptr);
  EXPECT_EQ(TypeParam::kFailOk, retval2);
  ASSERT_TRUE(manifest_ptr.get() != nullptr);
...
TYPED_TEST(T_ObjectFetcher, FetchLegacyHistorySlow) {
  ...
  typename TypeParam::Failures retval = object_fetcher->FetchHistory(
      &history, TestFixture::previous_history_hash);
  EXPECT_EQ(TypeParam::kFailOk, retval);
  ASSERT_NE(static_cast<typename TypeParam::HistoryTN *>(NULL), history)
      << "didn't find: "
...
TYPED_TEST(T_ObjectFetcher, FetchReflogSlow) {
  ...
  typename TypeParam::Failures retval = object_fetcher->FetchReflog(
      TestFixture::reflog_hash, &reflog);
  EXPECT_EQ(TypeParam::kFailOk, retval);
...
TYPED_TEST(T_ObjectFetcher, LoadBrokenDatabaseObjectsSlow) {
  ...
  std::unique_ptr<typename TypeParam::HistoryTN> history;
  typename TypeParam::Failures retval;
  const typename TypeParam::Failures
      expected_error = TestFixture::IsHttpObjectFetcher()
  ...
  EXPECT_FALSE(catalog.get() != nullptr);
  EXPECT_EQ(0u, TestFixture::CountTemporaryFiles());
