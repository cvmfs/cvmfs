...
class T_HashFilter : public ::testing::Test {
 public:
  // TODO(rmeusel): C++11 replace this custom classes by std::bind()

  class FillWrapper {
   public:
    explicit FillWrapper(HashFilterT *filter) : filter_(filter) { }
    void operator()(const shash::Any &hash) { filter_->Fill(hash); }
  ...
  class ContainsWrapper {
   public:
    explicit ContainsWrapper(HashFilterT *filter) : filter_(filter) { }
...
TYPED_TEST(T_HashFilter, Initialize) { TypeParam filter; }
...
TYPED_TEST(T_HashFilter, EmptyFilter) {
  ...
  EXPECT_FALSE(
      filter.Contains(sha("ac1ddc9c4283f5bb8db64c2e5771eeb44803399f")));
  EXPECT_FALSE(
      filter.Contains(rmd("da39a3ee5e6b4b0d3255bfef95601890afd80709")));
  EXPECT_FALSE(filter.Contains(md5("eb8a956c0a1164b84262505a629e8a1f")));

  EXPECT_FALSE(filter.Contains(
      sha("ed0e0b2c3590bb9e72f849ed6bebb97f06371abf", shash::kSuffixCatalog)));
  EXPECT_FALSE(filter.Contains(
      rmd("f66853b7a7731719f3babb59623b00b3e02a4a4f", shash::kSuffixHistory)));
  EXPECT_FALSE(filter.Contains(
      md5("c70ac2b76b5c25b07ae80ea572bb8265", shash::kSuffixCatalog)));
}
...
TYPED_TEST(T_HashFilter, FillSha1) {
  ...
  EXPECT_FALSE(
      filter.Contains(sha("8271640616eb764f5b87dcb5a4c3d9a2aadcd816")));
  EXPECT_FALSE(
      filter.Contains(sha("7d988a2f0539bcd5c9cce8ba349f0e15ae2da92e")));
  EXPECT_FALSE(
      filter.Contains(sha("50e2e699ca7e5edfb81de5a92a6555ba9852c3a7")));
}
...
TYPED_TEST(T_HashFilter, FillRmd160) {
  ...
  EXPECT_FALSE(
      filter.Contains(rmd("8271640616eb764f5b87dcb5a4c3d9a2aadcd816")));
  EXPECT_FALSE(
      filter.Contains(rmd("7d988a2f0539bcd5c9cce8ba349f0e15ae2da92e")));
  EXPECT_FALSE(
      filter.Contains(rmd("50e2e699ca7e5edfb81de5a92a6555ba9852c3a7")));
}
...
TYPED_TEST(T_HashFilter, FillHeterogeneous) {
  ...
  EXPECT_FALSE(
      filter.Contains(rmd("451afd372792933f4dbad535413346bfe7e7cc08")));
  EXPECT_FALSE(
      filter.Contains(rmd("2579075d95e9c7abfbedb78de5307a7f27aa7109")));
  EXPECT_FALSE(
      filter.Contains(rmd("40e938a032915acb48da226792d394905321fb9e")));
  EXPECT_FALSE(
      filter.Contains(sha("8773f6f313e91c7727fe9502dc3e926b4b70f065")));
  EXPECT_FALSE(
      filter.Contains(sha("c5aba768b5d33a7875e70a05e72e87bfdfb98933")));
  EXPECT_FALSE(
      filter.Contains(sha("ed737e36b7b881184cddb78d73e99d11ea2e948a")));
...
TYPED_TEST(T_HashFilter, DoubleInsert) {
  ...
  filter.Fill(
      sha("2e19e3667a617f9381357e580f935ee783a98025", shash::kSuffixPartial));
  filter.Fill(
      rmd("dd240ad846f9610d577778caed9410fdc3e385fa", shash::kSuffixPartial));
  ...
  EXPECT_TRUE(filter.Contains(
      sha("2e19e3667a617f9381357e580f935ee783a98025", shash::kSuffixPartial)));
  EXPECT_TRUE(filter.Contains(
      rmd("2e19e3667a617f9381357e580f935ee783a98025", shash::kSuffixPartial)));
  EXPECT_TRUE(filter.Contains(
      sha("dd240ad846f9610d577778caed9410fdc3e385fa", shash::kSuffixPartial)));
  EXPECT_TRUE(filter.Contains(
      rmd("dd240ad846f9610d577778caed9410fdc3e385fa", shash::kSuffixPartial)));
  EXPECT_TRUE(filter.Contains(
      md5("d985d0ea551c1253c2305140c583d11f", shash::kSuffixPartial)));

  EXPECT_TRUE(filter.Contains(
      sha("2e19e3667a617f9381357e580f935ee783a98025", shash::kSuffixCatalog)));
  EXPECT_TRUE(filter.Contains(
      rmd("2e19e3667a617f9381357e580f935ee783a98025", shash::kSuffixCatalog)));
  EXPECT_TRUE(filter.Contains(
      sha("dd240ad846f9610d577778caed9410fdc3e385fa", shash::kSuffixCatalog)));
  EXPECT_TRUE(filter.Contains(
      rmd("dd240ad846f9610d577778caed9410fdc3e385fa", shash::kSuffixCatalog)));
  EXPECT_TRUE(filter.Contains(
      md5("d985d0ea551c1253c2305140c583d11f", shash::kSuffixCatalog)));
}
...
TYPED_TEST(T_HashFilter, FillHeterogeneousWithSuffixes) {
  ...
  filter.Fill(sha("451afd372792933f4dbad535413346bfe7e7cc08"));
  filter.Fill(
      sha("451afd372792933f4dbad535413346bfe7e7cc08", shash::kSuffixPartial));
  filter.Fill(sha("2579075d95e9c7abfbedb78de5307a7f27aa7109"));
  filter.Fill(
      sha("2579075d95e9c7abfbedb78de5307a7f27aa7109", shash::kSuffixPartial));
  filter.Fill(sha("40e938a032915acb48da226792d394905321fb9e"));
  filter.Fill(
      sha("40e938a032915acb48da226792d394905321fb9e", shash::kSuffixPartial));
  filter.Fill(rmd("8773f6f313e91c7727fe9502dc3e926b4b70f065"));
  filter.Fill(
      rmd("8773f6f313e91c7727fe9502dc3e926b4b70f065", shash::kSuffixPartial));
  filter.Fill(rmd("c5aba768b5d33a7875e70a05e72e87bfdfb98933"));
  filter.Fill(
      rmd("c5aba768b5d33a7875e70a05e72e87bfdfb98933", shash::kSuffixPartial));
  filter.Fill(rmd("ed737e36b7b881184cddb78d73e99d11ea2e948a"));
  filter.Fill(
      rmd("ed737e36b7b881184cddb78d73e99d11ea2e948a", shash::kSuffixPartial));
  filter.Fill(md5("9731b9b024f890dd1ce81e54a5f9bfe7"));
  ...
  EXPECT_TRUE(filter.Contains(
      sha("451afd372792933f4dbad535413346bfe7e7cc08", shash::kSuffixCatalog)));
  EXPECT_TRUE(filter.Contains(
      sha("2579075d95e9c7abfbedb78de5307a7f27aa7109", shash::kSuffixCatalog)));
  EXPECT_TRUE(filter.Contains(
      sha("40e938a032915acb48da226792d394905321fb9e", shash::kSuffixCatalog)));
  EXPECT_TRUE(filter.Contains(
      rmd("8773f6f313e91c7727fe9502dc3e926b4b70f065", shash::kSuffixCatalog)));
  EXPECT_TRUE(filter.Contains(
      rmd("c5aba768b5d33a7875e70a05e72e87bfdfb98933", shash::kSuffixCatalog)));
  EXPECT_TRUE(filter.Contains(
      rmd("ed737e36b7b881184cddb78d73e99d11ea2e948a", shash::kSuffixCatalog)));
  EXPECT_TRUE(filter.Contains(
      md5("9731b9b024f890dd1ce81e54a5f9bfe7", shash::kSuffixCatalog)));
  EXPECT_TRUE(filter.Contains(
      md5("eb8a956c0a1164b84262505a629e8a1f", shash::kSuffixCatalog)));
  EXPECT_TRUE(filter.Contains(
      md5("b35a73d2b0cfe44018e9068f81abbb99", shash::kSuffixCatalog)));
  ...
  EXPECT_FALSE(filter.Contains(sha("ed737e36b7b881184cddb78d73e99d11ea2e948a",
                                   shash::kSuffixCertificate)));
  EXPECT_FALSE(filter.Contains(
      md5("148dd83c24e8de4377d1ecfa17d10617", shash::kSuffixCertificate)));
  EXPECT_FALSE(filter.Contains(
      md5("ed70ca83dc1aac21ce26355fd1bb9325", shash::kSuffixCertificate)));
  EXPECT_FALSE(filter.Contains(
      md5("f60447901653b1ddcc698d644b9675c8", shash::kSuffixCertificate)));
}
...
TYPED_TEST(T_HashFilter, FillManyRandomHashesSlow) {
  ...
  std::generate(random_hashes.begin(), random_hashes.end(),
