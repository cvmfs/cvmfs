...
TEST_F(CompressorTest, Compression) {
  ...
  // Compress the output
  bool deflate_finished = compressor->Deflate(buf, buf_size, (unsigned char*&)ptr_test_string, size_input, true);
  ...
  DecompressMem2Mem(buf, buf_size, (void**)&decompress_buf, &decompress_size);
  
  // Check if the string is the same as the beginning
  ASSERT_TRUE(strcmp(decompress_buf, test_string) == 0);
...
TEST_F(CompressorTest, EchoCompression) {
  ...
  // input.
  ASSERT_TRUE(strcmp((char*)buf, test_string) == 0);
