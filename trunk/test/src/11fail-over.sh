...
cvmfs_run_test() {
  ...
  service cvmfs restartclean >> $logfile 2>&1
  RETVAL=$[$RETVAL+$?]
  echo 'CVMFS_HTTP_PROXY="http://127.0.0.1:3130;http://127.0.0.1:3129"' >> /etc/cvmfs/config.d/127.0.0.1.conf  
  echo 'CVMFS_SERVER_URL="http://127.0.0.1:8081/catalogs"' >> /etc/cvmfs/config.d/127.0.0.1.conf
